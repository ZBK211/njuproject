...
0:40-1:10 运行 `python scripts/run_demo.py`，保留 `list_dir`、`read_file`、`write_file`、`run_command` 和 pytest 通过结果。

1:10-1:50 打开 `http://127.0.0.1:8787`，点击“运行一次”，重点展示网页中的本地工具调用参数、workspace 路径、代码 diff、pytest 输出、项目记忆和合规检查。若使用真实模型，切到 DeepSeek 模式并在密码框临时输入密钥，不要把密钥录入文档或提交到仓库。
