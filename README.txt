...
运行：`python -m pytest -q`。真实模型演示：设置 `DEEPSEEK_API_KEY` 后运行 `python scripts/run_deepseek_demo.py`，默认模型为 `deepseek-v4-flash`。可视化演示：`python scripts/demo_server.py` 后打开 `http://127.0.0.1:8787`，切到 DeepSeek 实时模型，页面会展示本地工具调用参数、代码 diff、pytest 输出和记忆文件。合规自检：`python scripts/audit_assignment.py`。
