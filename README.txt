...
运行：`python -m pytest -q`。离线演示：`python scripts/run_demo.py`。可视化演示：`python scripts/demo_server.py` 后打开 `http://127.0.0.1:8787`，页面会展示本地工具调用参数、代码 diff、pytest 输出和记忆文件；也可切到 DeepSeek 实时模型模式，密钥只在运行时输入或从环境变量读取。
